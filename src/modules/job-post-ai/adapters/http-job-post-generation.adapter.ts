...
/** Private client for JD generation; it never forwards browser credentials. */
@Injectable()
export class HttpJobPostGenerationAdapter implements JobPostGenerationProviderPort {
  private readonly logger = new Logger(HttpJobPostGenerationAdapter.name);
  ...
  async generateStructured(request: JobPostGenerationRequest): Promise<JobPostGenerationResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.warn(`upnext-ai JD generation returned ${response.status}; body omitted`, {
  ...
  private endpoint(): string {
    ...
    return new URL('/internal/v1/job-posts/generate', `${baseUrl}/`).toString();
  }
  ...
  private linkedSignal(external?: AbortSignal) {
    ...
    const timer = setTimeout(
      ...
      this.configService.get<number>('aiJobPostGenerationTimeoutMs') ?? 45_000,
    );
