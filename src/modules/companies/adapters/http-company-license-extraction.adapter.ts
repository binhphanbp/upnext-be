...
/** Private client for the one licence-extraction capability; never accepts browser JWTs. */
@Injectable()
export class HttpCompanyLicenseExtractionAdapter implements CompanyLicenseExtractionProviderPort {
  private readonly logger = new Logger(HttpCompanyLicenseExtractionAdapter.name);
  ...
  async extractStructured(
    ...
    try {
      ...
      if (!response.ok) {
        // A licence body can echo registration data, so only its size is logged.
        const body = await response.text().catch(() => '');
        this.logger.warn(`upnext-ai licence extraction returned ${response.status}; body omitted`, {
  ...
  private endpoint(): string {
    ...
    return new URL('/internal/v1/companies/license-extract', `${baseUrl}/`).toString();
  }
  ...
  private linkedSignal(external?: AbortSignal) {
    ...
    const timer = setTimeout(
      ...
      this.configService.get<number>('aiCompanyLicenseExtractionTimeoutMs') ?? 45_000,
    );
