...
async function runTest() {
  ...
  const postCreateRes = await fetch(`${BASE_URL}/admin/posts`, {
    ...
    body: JSON.stringify({
      ...
      content: 'Nội dung hướng dẫn chi tiết về xây dựng backend chuẩn RESTful API với NestJS và Prisma...',
      status: 'PUBLISHED',
      type: 'BLOG',
  ...
  const deletePostRes = await fetch(`${BASE_URL}/admin/posts/${postId}`, { method: 'DELETE', headers: authHeader });
  console.log('✅ 5.1 DELETE /admin/posts/:id -> Status:', deletePostRes.status);

  const deleteCatRes = await fetch(`${BASE_URL}/admin/post-categories/${categoryId}`, { method: 'DELETE', headers: authHeader });
  console.log('✅ 5.2 DELETE /admin/post-categories/:id -> Status:', deleteCatRes.status);

  const deleteTagRes = await fetch(`${BASE_URL}/admin/post-tags/${tagId}`, { method: 'DELETE', headers: authHeader });
